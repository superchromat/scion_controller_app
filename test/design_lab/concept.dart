...
class _DeskModel extends ChangeNotifier {
  ...
  double fieldU = 0.68, fieldV = 0.42; // colour-field puck, in source-local coords
  int warpN = 4; // WARP LUT mesh resolution (N×N vertices), adjustable
  List<Offset> warp = List<Offset>.filled(4 * 4, Offset.zero); // normalized offsets
  int warpRev = 0; // bumps on any warp change (in-place list can't be diffed by ref)
  ...
  void selectStage(String s) { stage = s; notifyListeners(); }
  void selectTool(String s) { tool = s; notifyListeners(); }
  ...
  void setWarpN(int n) {
    ...
    if (n == warpN) return;
    warpN = n;
    warp = List<Offset>.filled(n * n, Offset.zero); // resample would preserve work; reset for now
    warpRev++;
    notifyListeners();
  }
  void resetWarp() { for (var i = 0; i < warp.length; i++) warp[i] = Offset.zero; warpRev++; notifyListeners(); }
  void toggleFreeCorner(int i) {
    if (freeCorners.contains(i)) { freeCorners.remove(i); cornerWarp[i] = Offset.zero; }
    else { freeCorners.add(i); }
    warpRev++;
    notifyListeners();
  }
  ...
  void setRot(double d) { rotDeg = d.clamp(-180, 180); notifyListeners(); }
  void nudgeRot(double d) { rotDeg = (rotDeg + d).clamp(-180, 180); notifyListeners(); }
...
class _GeoLayout {
  ...
  final List<Offset> cropEdges; // top,right,bottom,left midpoints (CROP handles)
...
_GeoLayout _computeGeo(Size size, {
  ...
  required double fu, required double fv,
  List<Offset>? warp, int n = 4, List<Offset>? cornerWarp,
}) {
  final base = size.center(Offset.zero);
  final c = base + Offset((px - 0.5) * size.width * 0.5, (py - 0.5) * size.height * 0.5);
  ...
  Offset corner(int i, double bx, double by) {
    var p = rot(bx, by);
    final o = (cornerWarp != null && cornerWarp.length == 4) ? cornerWarp[i] : Offset.zero;
    if (o != Offset.zero) {
  ...
  final full = [corner(0, -fw / 2, -fh / 2), corner(1, fw / 2, -fh / 2), corner(2, fw / 2, fh / 2), corner(3, -fw / 2, fh / 2)];
  Offset lerp(Offset a, Offset b, double t) => Offset(a.dx + (b.dx - a.dx) * t, a.dy + (b.dy - a.dy) * t);
  Offset bilerp(List<Offset> q, double u, double v) => lerp(lerp(q[0], q[1], u), lerp(q[3], q[2], u), v);
  final quad = [bilerp(full, cl, ct), bilerp(full, 1 - cr, ct), bilerp(full, 1 - cr, 1 - cb), bilerp(full, cl, 1 - cb)];
  final cropEdges = [lerp(quad[0], quad[1], 0.5), lerp(quad[1], quad[2], 0.5), lerp(quad[2], quad[3], 0.5), lerp(quad[3], quad[0], 0.5)];
  ...
  for (int j = 0; j < n; j++) {
    for (int i = 0; i < n; i++) {
      ...
      final off = (warp != null && warp.length == n * n) ? warp[j * n + i] : Offset.zero;
      final dx = off.dx * fw, dy = off.dy * fh; // local displacement
      mesh.add(baseP + Offset(dx * ca - dy * sa, dx * sa + dy * ca));
    }
  }
  return _GeoLayout(c, ang, fw, fh, full, quad, cropEdges, mesh, grip, bilerp(full, fu, fv));
}
...
class _IKnob extends StatelessWidget {
  ...
  final double? size;
  final String fmt;
  final double def; // default/neutral — draws the detent + is the double-tap reset target
  ...
  Widget build(BuildContext context) {
    ...
    return Column(mainAxisSize: MainAxisSize.min, children: [
      RotaryKnob(
        value: value, minValue: min, maxValue: max, isBipolar: bipolar,
        size: size ?? t.knobSm, format: fmt, onChanged: onChanged,
        defaultValue: def, neutralValue: def,
      ),
...
const List<double> _posterTh = [0.10, 0.18, 0.26, 0.34, 0.42, 0.5, 0.58, 0.66, 0.74, 0.82, 0.90];
...
class _SignalDeskBody extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    Widget instrument;
    switch (stage) {
      case 'geometry': instrument = const _GeometryInstrument(); break;
      case 'tone': instrument = const _ToneInstrument(); break;
      default: instrument = _PlaceholderInstrument(stage);
    }
    return Container(
      color: _bg,
    ...
    return LabeledCard(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.stretch, mainAxisSize: MainAxisSize.min, children: [
        ...
        Row(mainAxisAlignment: MainAxisAlignment.spaceEvenly, children: [
          _IKnob('Scale X', m.scaleX, 0.1, 2, (v) => m.setScale(x: v), def: 1),
          _IKnob('Scale Y', m.scaleY, 0.1, 2, (v) => m.setScale(y: v), def: 1),
          _IKnob('Pos X', m.posX, 0, 1, (v) => m.setPos(x: v), def: 0.5),
          _IKnob('Pos Y', m.posY, 0, 1, (v) => m.setPos(y: v), def: 0.5),
          _IKnob('Rotate', m.rotDeg, -180, 180, m.setRot, bipolar: true, fmt: '%.0f', def: 0),
        ]),
        SizedBox(height: t.md),
        ...
        SizedBox(height: t.md),
        Text('Canvas: outer corners scale (double-click a corner to free it for keystone/shear) · inner bars crop · grip rotates. Tools: Move · Warp (press-drag for mesh size) · Field.',
...
class _StatusStrip extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final t = GridProvider.of(context);
    Widget stat(String k, String v, {Color c = _ink}) => Row(mainAxisSize: MainAxisSize.min, children: [
          Text('$k ', style: t.textCaption.copyWith(color: _dim)),
          Text(v, style: t.textValue.copyWith(color: c, fontSize: t.u * 1.2)),
        ]);
    return Row(children: [
      Text('SCION', style: t.textTitle.copyWith(fontWeight: FontWeight.w700, letterSpacing: 2)),
      SizedBox(width: t.xs),
      Text('SEND 1', style: t.textTitle.copyWith(color: _amber, letterSpacing: 2)),
      SizedBox(width: t.lg),
      Container(
        padding: EdgeInsets.symmetric(horizontal: t.sm, vertical: t.xs * 0.6),
        decoration: BoxDecoration(color: _green.withValues(alpha: 0.14), borderRadius: BorderRadius.circular(3)),
        child: Row(mainAxisSize: MainAxisSize.min, children: [
          Container(width: t.xs, height: t.xs, decoration: const BoxDecoration(color: _green, shape: BoxShape.circle)),
          SizedBox(width: t.xs), Text('LOCKED', style: t.textCaption.copyWith(color: _green, letterSpacing: 1.5)),
        ]),
      ),
      SizedBox(width: t.lg),
      stat('IN', 'HDMI 1 · 3840×2160p60'),   // real: /input status
      SizedBox(width: t.lg),
      stat('OUT', '1920×1080p59.94', c: _green), // real: /output/resolution + /framerate
      const Spacer(),
    ]);
  }
}
...
class _SignalChain extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
      Text('SIGNAL CHAIN', style: t.textCaption.copyWith(letterSpacing: 1.5, color: _dim)),
      SizedBox(height: t.sm),
      Expanded(
        child: Column(
          children: [
            for (final s in stages)
              Expanded(child: Padding(
                ...
                child: GestureDetector(
                  ...
                  onTap: () => context.read<_DeskModel>().selectStage(keyOf(s.$1)),
                  child: _ChainNode(
                    label: s.$1, sub: s.$2, stage: s.$3,
                    active: m.stage == keyOf(s.$1), hasPreview: s.$5),
                ),
              )),
          ],
        ),
...
class _ChainNode extends StatelessWidget {
  ...
  const _ChainNode({required this.label, required this.sub, required this.stage, required this.active, this.hasPreview = false});
  @override
  Widget build(BuildContext context) {
    final t = GridProvider.of(context);
    return DecoratedBox(
      decoration: BoxDecoration(
        ...
        border: Border.all(color: active ? _amber.withValues(alpha: 0.8) : Colors.white.withValues(alpha: 0.05), width: active ? 1.2 : 1),
        boxShadow: active ? [BoxShadow(color: _amber.withValues(alpha: 0.10), blurRadius: 12, spreadRadius: 1)] : null,
      ),
      child: Padding(
        ...
        child: Row(children: [
          ...
          SizedBox(
            ...
            child: ClipRRect(
              ...
              child: hasPreview
                  ? CustomPaint(painter: _FramePainter(stage: stage, pixel: true))
                  : CustomPaint(painter: _ChainSchematicPainter(active: active)),
            ),
          ),
          SizedBox(width: t.sm),
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, mainAxisAlignment: MainAxisAlignment.center, children: [
            Text(label, maxLines: 1, overflow: TextOverflow.ellipsis, style: t.textLabel.copyWith(
              color: active ? _amber : _ink, fontWeight: FontWeight.w700, letterSpacing: 0.5)),
            Text(sub, maxLines: 1, overflow: TextOverflow.ellipsis, style: t.textCaption.copyWith(color: _dim)),
          ])),
        ]),
      ),
    );
...
class _ChainSchematicPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    canvas.drawRect(Offset.zero & size, Paint()..color = const Color(0xFF0E0E10));
    // dotted frame bounds
    ...
    Offset rot(double dx, double dy) => c + Offset(dx * math.cos(ang) - dy * math.sin(ang), dx * math.sin(ang) + dy * math.cos(ang));
    final w = size.width * 0.34, h = size.height * 0.42;
    final quad = [rot(-w, -h), rot(w, -h), rot(w, h), rot(-w, h)];
    canvas.drawPath(Path()..addPolygon(quad, true),
        Paint()..style = PaintingStyle.stroke..strokeWidth = 1.2..color = col.withValues(alpha: 0.9));
    canvas.drawLine(quad[0], quad[2], Paint()..color = col.withValues(alpha: 0.25)..strokeWidth = 0.75);
  }
  @override
  bool shouldRepaint(covariant _ChainSchematicPainter old) => old.active != active;
}
...
class _HeroCanvas extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
      Row(children: [
        Text('PROGRAM', style: t.textCaption.copyWith(letterSpacing: 1.5, color: _dim)),
        const Spacer(),
      ...
      Padding(
        ...
        child: GestureDetector(
          ...
          child: Container(
            ...
            child: Row(mainAxisSize: MainAxisSize.min, children: [
              Icon(ic, size: t.u * 1.4, color: key == selected ? Colors.black : _dim),
              SizedBox(width: t.xs),
              Text(label, style: t.textCaption.copyWith(
                  color: key == selected ? Colors.black : _dim, fontWeight: FontWeight.w600)),
            ]),
          ),
        ),
      );
...
class _WarpToolButtonState extends State<_WarpToolButton> {
  ...
  void _open(_DeskModel m) {
    ...
    _menuRect = Rect.fromLTWH(tl.dx, tl.dy + box.size.height + 4, w, sizes.length * _itemH);
    _hover = sizes.indexOf(m.warpN);
    _entry = OverlayEntry(builder: (_) => _menu(m));
    Overlay.of(context).insert(_entry!);
  }

  void _close() { _entry?.remove(); _entry = null; _hover = -1; }
  ...
  Widget _menu(_DeskModel m) {
    final t = GridProvider.of(context);
    return Positioned(
      left: _menuRect.left, top: _menuRect.top, width: _menuRect.width,
      child: Material(
        ...
        child: Container(
          ...
          child: Column(mainAxisSize: MainAxisSize.min, children: [
            for (int i = 0; i < sizes.length; i++)
              Container(
                height: _itemH, width: double.infinity, alignment: Alignment.center,
                color: i == _hover ? _amber : (sizes[i] == m.warpN ? _amber.withValues(alpha: 0.18) : Colors.transparent),
                child: Text('${sizes[i]}×${sizes[i]}', style: t.textLabel.copyWith(
                    color: i == _hover ? Colors.black : _ink, fontWeight: FontWeight.w600)),
              ),
          ]),
        ),
      ),
    ...
    return Padding(
      ...
      child: Listener(
        ...
        onPointerMove: (e) {
          ...
          if (i != _hover) { _hover = i; _entry?.markNeedsBuild(); }
        },
        onPointerUp: (_) { if (_hover >= 0) m.setWarpN(sizes[_hover]); _close(); },
        child: Container(
          key: _key,
          padding: EdgeInsets.symmetric(horizontal: t.sm, vertical: t.xs * 0.8),
          decoration: BoxDecoration(color: on ? _amber : _panel, borderRadius: BorderRadius.circular(4)),
          child: Row(mainAxisSize: MainAxisSize.min, children: [
            Icon(Icons.grid_on, size: t.u * 1.4, color: on ? Colors.black : _dim),
            SizedBox(width: t.xs),
            Text('Warp', style: t.textCaption.copyWith(color: on ? Colors.black : _dim, fontWeight: FontWeight.w600)),
            SizedBox(width: t.xs * 0.8),
            Text('${m.warpN}', style: t.textCaption.copyWith(color: on ? Colors.black : _ink, fontWeight: FontWeight.w700)),
            Icon(Icons.arrow_drop_down, size: t.u * 1.4, color: on ? Colors.black : _dim),
          ]),
        ),
      ),
    );
...
class _ToneInstrument extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return LabeledCard(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.stretch, mainAxisSize: MainAxisSize.min, children: [
        ...
        Text('LUMA  ·  256-bin histogram (MDINWFM)  +  grade curve', style: t.textCaption.copyWith(letterSpacing: 1.0, color: _dim)),
        SizedBox(height: t.xs),
        SizedBox(height: t.u * 6.6, child: CustomPaint(painter: _HistogramPainter(), size: Size.infinite)),
        // posterize band strip pinned to the SAME width/axis, directly below
        SizedBox(height: t.xs * 0.6),
        SizedBox(height: t.u * 2.6, child: CustomPaint(painter: _BandStripPainter(), size: Size.infinite)),
        SizedBox(height: t.xs),
        _axisRuler(t),
        SizedBox(height: t.md),
        ...
        SizedBox(height: t.xs),
        Row(children: [
          Expanded(child: _GradeZone(name: 'Shadows', angle: 2.4, mag: 0.5, con: 0.55, sat: 0.6)),
          SizedBox(width: t.sm),
          Expanded(child: _GradeZone(name: 'Midtones', angle: 0.6, mag: 0.28, con: 0.5, sat: 0.5)),
          SizedBox(width: t.sm),
          Expanded(child: _GradeZone(name: 'Highlights', angle: -1.1, mag: 0.7, con: 0.6, sat: 0.45)),
        ]),
        SizedBox(height: t.sm),
        // master picture trio — global, distinct from the per-zone Con/Sat
        KnobPanel('Master', const [('Contrast', 0.55), ('Saturation', 0.62), ('Hue', 0.5)], bipolar: true, knobSize: t.knobSm),
        SizedBox(height: t.sm),
  ...
  Widget _axisRuler(GridTokens t) => Row(children: [
        for (final l in const ['0', '', '64', '', '128', '', '192', '', '255'])
          Expanded(child: Text(l, textAlign: l == '0' ? TextAlign.left : (l == '255' ? TextAlign.right : TextAlign.center),
              style: t.textCaption.copyWith(color: _dim, fontSize: t.u * 0.75))),
      ]);
}
  ...
  Widget build(BuildContext context) {
    ...
    Widget pill(String label, bool active) => Container(
          ...
          decoration: BoxDecoration(
            ...
            borderRadius: BorderRadius.circular(4),
            border: Border.all(color: active ? _amber : Colors.white.withValues(alpha: 0.08)),
          ),
          child: Row(mainAxisSize: MainAxisSize.min, children: [
            Container(width: t.xs, height: t.xs, decoration: BoxDecoration(color: active ? _amber : _dim, shape: BoxShape.circle)),
            SizedBox(width: t.xs),
            Text(label, style: t.textCaption.copyWith(color: active ? _amber : _dim, fontWeight: FontWeight.w600, letterSpacing: 0.5)),
          ]),
        );
    Widget btn(String label) => Container(
          padding: EdgeInsets.symmetric(horizontal: t.sm, vertical: t.xs * 0.7),
          decoration: BoxDecoration(color: _panel, borderRadius: BorderRadius.circular(4), border: Border.all(color: Colors.white.withValues(alpha: 0.1))),
          child: Text(label, style: t.textCaption.copyWith(color: _ink)),
        );
    return Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
      Text('3-D ENGINE', style: t.textCaption.copyWith(letterSpacing: 1.5, color: _dim)),
      SizedBox(height: t.xs),
      Row(children: [
        pill('Grade', true),
        Padding(padding: EdgeInsets.symmetric(horizontal: t.xs), child: Text('⇄', style: t.textLabel.copyWith(color: _dim))),
        pill('.cube', false),
        const Spacer(),
        btn('Load…'), SizedBox(width: t.xs), btn('Export'),
      ]),
      SizedBox(height: t.xs * 0.6),
...
class _GradeZone extends StatelessWidget {
  ...
  const _GradeZone({required this.name, required this.angle, required this.mag, required this.con, required this.sat});
  @override
  Widget build(BuildContext context) {
    final t = GridProvider.of(context);
    return NeumorphicInset(
      ...
      padding: EdgeInsets.all(t.xs),
      child: Column(mainAxisSize: MainAxisSize.min, children: [
        Text(name.toUpperCase(), style: t.textCaption.copyWith(color: _dim, letterSpacing: 0.8, fontSize: t.u * 0.82)),
        SizedBox(height: t.xs),
        SizedBox(width: t.u * 3.9, height: t.u * 3.9, child: CustomPaint(painter: _TrackballPainter(angle, mag))),
        SizedBox(height: t.xs),
        Row(mainAxisAlignment: MainAxisAlignment.spaceEvenly, children: [
...
class _Trackball extends StatelessWidget {
  ...
  const _Trackball({required this.label, required this.size, required this.angle, required this.mag});
  @override
  Widget build(BuildContext context) {
    final t = GridProvider.of(context);
    return Column(mainAxisSize: MainAxisSize.min, children: [
      SizedBox(width: size, height: size, child: CustomPaint(painter: _TrackballPainter(angle, mag))),
      SizedBox(height: t.xs * 0.8),
      Text(label, style: t.textLabel.copyWith(color: _dim)),
    ]);
  }
  ...
  void paint(Canvas canvas, Size size) {
    ...
    canvas.drawCircle(c, r - 1.5, Paint()
      ...
      ..shader = const SweepGradient(colors: [
        Color(0xFFB05858), Color(0xFFB0A050), Color(0xFF58B070),
        Color(0xFF50A0B0), Color(0xFF6858B0), Color(0xFFB058A0), Color(0xFFB05858),
      ]).createShader(Rect.fromCircle(center: c, radius: r))
      ..color = Colors.white.withValues(alpha: 0.5));
    // inner ring guide
    canvas.drawCircle(c, r * 0.62, Paint()..style = PaintingStyle.stroke..strokeWidth = 0.75..color = Colors.white.withValues(alpha: 0.10));
    // centre reference
    canvas.drawCircle(c, 1.5, Paint()..color = Colors.white.withValues(alpha: 0.25));
    // position node + connecting line
    final node = c + Offset(math.cos(angle), math.sin(angle)) * r * 0.62 * mag;
    canvas.drawLine(c, node, Paint()..color = _amber.withValues(alpha: 0.6)..strokeWidth = 1);
    canvas.drawCircle(node, r * 0.14, Paint()..color = Colors.white);
    canvas.drawCircle(node, r * 0.14, Paint()..style = PaintingStyle.stroke..strokeWidth = 1..color = _amber);
  }
  @override
  bool shouldRepaint(covariant _TrackballPainter old) => old.angle != angle || old.mag != mag;
}
...
class _FramePainter extends CustomPainter {
  ...
  void _scene(Canvas canvas, Size size, {double sunY = 0.60}) {
    ...
    canvas.drawRect(r, Paint()..shader = const LinearGradient(
      ...
      colors: [Color(0xFF2A1A4A), Color(0xFF7A2E6A), Color(0xFFE86A4A), Color(0xFFF0B060)],
      stops: [0, 0.42, 0.66, 0.78],
    ).createShader(r));
    final sunC = Offset(size.width * 0.5, size.height * sunY);
    final sunR = size.height * 0.30;
    canvas.drawCircle(sunC, sunR, Paint()..shader = const LinearGradient(
      begin: Alignment.topCenter, end: Alignment.bottomCenter,
    ...
    for (double y = sunC.dy; y < sunC.dy + sunR; y += size.height * 0.05) {
      canvas.drawRect(Rect.fromLTWH(0, y, size.width, size.height * 0.02 + 0.6), gap);
    }
    final horizon = size.height * (sunY + 0.06);
    canvas.drawRect(Rect.fromLTRB(0, horizon, size.width, size.height), Paint()..color = const Color(0xFF150A20));
    final grid = Paint()..color = const Color(0xFF7A5AE0).withValues(alpha: 0.55)..strokeWidth = 1;
    final vp = Offset(size.width * 0.5, horizon);
    for (int i = -6; i <= 6; i++) {
      canvas.drawLine(Offset(size.width * 0.5 + i * size.width * 0.16, size.height), vp, grid);
    }
    for (int i = 1; i <= 5; i++) {
      final y = horizon + math.pow(i / 5.0, 2.2) * (size.height - horizon);
      canvas.drawLine(Offset(0, y.toDouble()), Offset(size.width, y.toDouble()), grid);
    }
  }
  ...
  void _render(Canvas canvas, Size size) {
    ...
    switch (stage) {
      case 0: // INPUT — raw 4K source, letterboxed to show over-scan
        _scene(canvas, size);
        canvas.drawRect(Rect.fromLTWH(0, 0, size.width, size.height * 0.08), Paint()..color = Colors.black);
        canvas.drawRect(Rect.fromLTWH(0, size.height * 0.92, size.width, size.height * 0.08), Paint()..color = Colors.black);
      case 1: // GEOMETRY — reframed: scaled + rotated, tighter crop
        ...
        _scene(canvas, size);
        canvas.restore();
        canvas.drawRect(r.deflate(2), Paint()..style = PaintingStyle.stroke..strokeWidth = 1.5..color = _amber);
      ...
      case 4: // GENERATE — colour-field rings + text glyph composited
        _scene(canvas, size);
        for (int i = 1; i <= 3; i++) {
          canvas.drawCircle(Offset(size.width * 0.66, size.height * 0.4), size.height * 0.12 * i,
              Paint()..style = PaintingStyle.stroke..strokeWidth = 1.5..color = _green.withValues(alpha: 0.7 - i * 0.12));
        }
        final tp = TextPainter(
            text: TextSpan(text: 'Aa', style: TextStyle(color: Colors.white, fontFamily: 'DINPro', fontWeight: FontWeight.w700, fontSize: size.height * 0.34)),
            textDirection: TextDirection.ltr)..layout();
        tp.paint(canvas, Offset(size.width * 0.10, size.height * 0.14));
      default: // OUTPUT — final graded, punchy contrast + vignette
        _scene(canvas, size);
        canvas.drawRect(r, Paint()..blendMode = BlendMode.overlay..color = Colors.black.withValues(alpha: 0.22));
        canvas.drawRect(r, Paint()..shader = RadialGradient(
          colors: [Colors.transparent, Colors.black.withValues(alpha: 0.4)], stops: const [0.55, 1]).createShader(r));
    }
  }
  ...
  void _posterized(Canvas canvas, Size size) {
    ...
    _scene(canvas, size);
    const bands = [
      Color(0xFF201048), Color(0xFF5A2A70), Color(0xFFC03E58),
      Color(0xFFE86A4A), Color(0xFFF0B050), Color(0xFFF0E080),
    ];
    ...
    for (int i = 0; i < bands.length; i++) {
      ...
      canvas.drawRect(Rect.fromLTRB(0, y0, size.width, y1),
          Paint()..blendMode = BlendMode.hardLight..color = bands[i].withValues(alpha: 0.5));
    }
  }
  ...
  void _blurred(Canvas canvas, Size size) {
    ...
    final cols = [const Color(0xFFE86A4A), const Color(0xFF7A2E6A), const Color(0xFFF0B060)];
    final rnd = math.Random(9);
    for (int i = 0; i < 10; i++) {
      blocks.color = cols[i % 3].withValues(alpha: 0.25);
      final w = size.width * (0.3 + rnd.nextDouble() * 0.4);
      canvas.drawRect(Rect.fromLTWH(rnd.nextDouble() * size.width - w / 2, rnd.nextDouble() * size.height, w, w * 0.5), blocks);
    }
    canvas.drawRect(Offset.zero & size, Paint()..color = Colors.white.withValues(alpha: 0.04));
    final g = Paint();
    for (int i = 0; i < 180; i++) {
      g.color = Colors.white.withValues(alpha: rnd.nextDouble() * 0.10);
      canvas.drawRect(Rect.fromLTWH(rnd.nextDouble() * size.width, rnd.nextDouble() * size.height, 1.2, 1.2), g);
    }
  }
...
class _OverlayPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    // crop rect inset
    final crop = Rect.fromLTRB(size.width * 0.08, size.height * 0.10, size.width * 0.86, size.height * 0.92);
    final dim = Path()
      ..addRect(Offset.zero & size)
      ..addRect(crop)
      ..fillType = PathFillType.evenOdd;
    canvas.drawPath(dim, Paint()..color = Colors.black.withValues(alpha: 0.45));
    canvas.drawRect(crop, Paint()..style = PaintingStyle.stroke..strokeWidth = 1..color = Colors.white.withValues(alpha: 0.9));
    // corner handles
    ...
    // rule-of-thirds
    final third = Paint()..color = Colors.white.withValues(alpha: 0.18)..strokeWidth = 0.75;
    for (int i = 1; i < 3; i++) {
    ...
    canvas.drawCircle(puck, 13, Paint()..style = PaintingStyle.stroke..strokeWidth = 2..color = _green);
    canvas.drawCircle(puck, 4, Paint()..color = _green);
    // orientation handle
    final ang = -0.6;
    canvas.drawLine(puck, puck + Offset(math.cos(ang), math.sin(ang)) * 34,
        Paint()..color = _green..strokeWidth = 1.5);
    canvas.drawCircle(puck + Offset(math.cos(ang), math.sin(ang)) * 34, 3.5, Paint()..color = _green);
  }
  @override
...
class _GeometryStagePainter extends CustomPainter {
  ...
  _GeometryStagePainter({
    required this.rotDeg, required this.cl, required this.cr, required this.ct, required this.cb,
    required this.sx, required this.sy, required this.px, required this.py,
    required this.fu, required this.fv, required this.tool, required this.warp,
    required this.warpN, required this.warpRev, required this.cornerWarp, required this.freeCorners,
    required this.stageSize, required this.stageOffset, this.hot,
  });
  @override
  void paint(Canvas canvas, Size size) {
    ...
    for (int i = 1; i < 16; i++) {
      canvas.drawLine(Offset(ss.width * i / 16, 0), Offset(ss.width * i / 16, ss.height), grid);
    }
    for (int i = 1; i < 9; i++) {
      canvas.drawLine(Offset(0, ss.height * i / 9), Offset(ss.width, ss.height * i / 9), grid);
    }
    canvas.drawRect(r.deflate(1), Paint()
      ..style = PaintingStyle.stroke..strokeWidth = 1..color = Colors.white.withValues(alpha: 0.18));

    final g = _computeGeo(ss, rotDeg: rotDeg, cl: cl, cr: cr, ct: ct, cb: cb, sx: sx, sy: sy, px: px, py: py, fu: fu, fv: fv, warp: warp, n: warpN, cornerWarp: cornerWarp);
    Offset lerp(Offset a, Offset b, double t) => Offset(a.dx + (b.dx - a.dx) * t, a.dy + (b.dy - a.dy) * t);
    final warping = tool == 'warp';

    // ghost = full source frame (SCALE handles at its corners)
    canvas.drawPath(Path()..addPolygon(g.full, true), Paint()
      ..style = PaintingStyle.stroke..strokeWidth = 1..color = Colors.white.withValues(alpha: 0.18));
    ...
    canvas.restore();
    canvas.drawPath(qp, Paint()..style = PaintingStyle.stroke..strokeWidth = 1.5..color = _amber);
    ...
    for (int i = 0; i < 4; i++) {
      ...
      final col = (on ? Colors.white : (free ? const Color(0xFFF0B830) : const Color(0xFF9AA6FF))).withValues(alpha: on ? 1 : aa);
      if (free) {
        final s = on ? 9.0 : 7.0;
        canvas.save();
        canvas.translate(g.full[i].dx, g.full[i].dy);
        canvas.rotate(math.pi / 4);
        canvas.drawRect(Rect.fromCenter(center: Offset.zero, width: s, height: s), Paint()..color = col);
        canvas.restore();
      } else {
    ...
    for (int i = 0; i < 4; i++) {
      ...
      canvas.drawRRect(RRect.fromRectAndRadius(
        Rect.fromCenter(center: Offset.zero, width: len, height: thick), const Radius.circular(2)),
        Paint()..color = (on ? Colors.white : _amber).withValues(alpha: on ? 1 : aa));
      canvas.restore();
    }
    // colour-field puck — only under the Field tool (its own concern)
    if (tool == 'field') {
      canvas.drawCircle(g.puck, hot == 'field' ? 13 : 11, Paint()..style = PaintingStyle.stroke..strokeWidth = 2..color = hot == 'field' ? Colors.white : _green);
  ...
  void _drawWarpMesh(Canvas canvas, _GeoLayout g) {
    final n = warpN;
    final line = Paint()..color = const Color(0xFF66E0C0).withValues(alpha: 0.55)..strokeWidth = 1;
    ...
    for (int k = 0; k < g.mesh.length; k++) {
      final on = hot == 'w$k';
      canvas.drawCircle(g.mesh[k], on ? 6 : 4, Paint()..color = on ? Colors.white : const Color(0xFF66E0C0));
    }
  }

  @override
  bool shouldRepaint(covariant _GeometryStagePainter o) =>
      o.rotDeg != rotDeg || o.cl != cl || o.cr != cr || o.ct != ct || o.cb != cb ||
      o.sx != sx || o.sy != sy || o.px != px || o.py != py || o.fu != fu || o.fv != fv ||
      o.hot != hot || o.tool != tool || o.warpRev != warpRev ||
      o.stageSize != stageSize || o.stageOffset != stageOffset;
}
...
class _GeometryCanvasState extends State<_GeometryCanvas> {
  String? _drag; // 's0'..'s3' scale, 'e0'..'e3' crop, 'rot', 'field', 'w0'.. warp, 'body'
  ...
  _GeoLayout _geo(_DeskModel m) => _computeGeo(_size,
      rotDeg: m.rotDeg, cl: m.cl, cr: m.cr, ct: m.ct, cb: m.cb,
      sx: m.scaleX, sy: m.scaleY, px: m.posX, py: m.posY, fu: m.fieldU, fv: m.fieldV,
      warp: m.warp, n: m.warpN, cornerWarp: m.cornerWarp);
  ...
  String? _hitTest(_DeskModel m, Offset p) {
    ...
    for (int i = 0; i < 4; i++) {
      if ((p - g.cropEdges[i]).distance < thresh) return 'e$i'; // crop (inner edges)
    }
    for (int i = 0; i < 4; i++) {
      if ((p - g.full[i]).distance < thresh) return 's$i'; // scale (outer corners)
    }
    return null;
  }
  ...
  void _apply(_DeskModel m, Offset p) {
    ...
    if (d == 'rot') {
      ...
      m.setPos(x: _px0 + (p.dx - _p0.dx) / _size.width, y: _py0 + (p.dy - _p0.dy) / _size.height);
    } else if (d.startsWith('e')) {
      // crop via inner edge handle
      final i = int.parse(d.substring(1));
      final (u, v) = _invMap(p, g);
      switch (i) {
        case 0: m.setCrop(t: v); break;        // top
        case 1: m.setCrop(r: 1 - u); break;    // right
        case 2: m.setCrop(b: 1 - v); break;    // bottom
        case 3: m.setCrop(l: u); break;        // left
      }
    } else if (d.startsWith('s')) {
      // scale via outer frame corner — centre-anchored
      final o = _unrot(p, g); // frame-local pixels from centre
      m.setScale(x: 4 * o.dx.abs() / _size.width, y: 2 * o.dy.abs() / (_size.height * 0.62));
      ...
      final i = int.parse(d.substring(1));
      final baseGeo = _computeGeo(_size, rotDeg: m.rotDeg, cl: m.cl, cr: m.cr, ct: m.ct, cb: m.cb,
          sx: m.scaleX, sy: m.scaleY, px: m.posX, py: m.posY, fu: m.fieldU, fv: m.fieldV); // no cornerWarp
      ...
      final off = _unrot(p - base + g.center, g); // displacement in local pixels
        ...
        child: MouseRegion(
          ...
          child: GestureDetector(
            ...
            onPanStart: (dg) {
              ...
              setState(() {
                if (hit != null && hit.startsWith('s') && m.freeCorners.contains(int.parse(hit.substring(1)))) {
                  _drag = 'k${hit.substring(1)}'; // freed corner → keystone drag
                } else if (hit != null) {
                  _drag = hit;
                } else if (m.tool == 'move') {
                  _drag = 'body'; _p0 = p; _px0 = m.posX; _py0 = m.posY;
                } else {
                  _drag = null;
                }
              });
            ...
            child: CustomPaint(
              ...
              painter: _GeometryStagePainter(
                rotDeg: m.rotDeg, cl: m.cl, cr: m.cr, ct: m.ct, cb: m.cb,
                sx: m.scaleX, sy: m.scaleY, px: m.posX, py: m.posY,
                fu: m.fieldU, fv: m.fieldV, tool: m.tool, warp: m.warp, warpN: m.warpN,
                warpRev: m.warpRev, cornerWarp: m.cornerWarp, freeCorners: m.freeCorners,
                stageSize: _size, stageOffset: _stageOffset, hot: hot),
            ),
          ),
        ),
      );
...
class _HistogramPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    final r = Offset.zero & size;
    canvas.drawRRect(RRect.fromRectAndRadius(r, const Radius.circular(4)), Paint()..color = const Color(0xFF0E0E10));
    ...
    canvas.drawPath(curve, Paint()..style = PaintingStyle.stroke..strokeWidth = 1.6..color = _amber);
    // posterize thresholds as vertical ticks on the SAME axis as the strip below
    for (final x in _posterTh) {
      canvas.drawLine(Offset(x * size.width, 0), Offset(x * size.width, size.height),
          Paint()..color = Colors.white.withValues(alpha: 0.22)..strokeWidth = 1);
    }
  }
  double _smooth(double x, double k) => math.pow(x, 1 / (1.6)) * (1 - k) + x * k;
  @override
  bool shouldRepaint(covariant CustomPainter old) => false;
}
...
class _BandStripPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    const colors = [
      Color(0xFF101018), Color(0xFF2A2050), Color(0xFF5A2A70), Color(0xFF8A2E6A),
      Color(0xFFC03E58), Color(0xFFE86A4A), Color(0xFFF0A050), Color(0xFFF0D060),
      Color(0xFFE0E080), Color(0xFFB8E0A0), Color(0xFF80D0C0), Color(0xFFC0E0F0),
    ];
    for (int i = 0; i < th.length - 1; i++) {
      final x0 = th[i] * size.width, x1 = th[i + 1] * size.width;
      canvas.drawRect(Rect.fromLTRB(x0, 0, x1, size.height), Paint()..color = colors[i % colors.length]);
    }
    // dividers with nubs
    final dp = Paint()..color = Colors.white.withValues(alpha: 0.85)..strokeWidth = 1;
...
class _Sparkline extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) => CustomPaint(painter: _SparkPainter(seed, color));
}
...
class _SparkPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    canvas.drawPath(p, Paint()..style = PaintingStyle.stroke..strokeWidth = 1..color = color.withValues(alpha: 0.8));
  }
  @override
  bool shouldRepaint(covariant CustomPainter old) => false;
}
...
class _CaptureStatus extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Row(children: [
      Text('SCION', style: t.textTitle.copyWith(fontWeight: FontWeight.w700, letterSpacing: 2)),
      SizedBox(width: t.xs),
      Text('RETURN', style: t.textTitle.copyWith(color: const Color(0xFF5AB0E0), letterSpacing: 2)),
      SizedBox(width: t.lg),
      Container(
        padding: EdgeInsets.symmetric(horizontal: t.sm, vertical: t.xs * 0.6),
        decoration: BoxDecoration(color: _green.withValues(alpha: 0.14), borderRadius: BorderRadius.circular(3)),
        child: Row(mainAxisSize: MainAxisSize.min, children: [
          Container(width: t.xs, height: t.xs, decoration: const BoxDecoration(color: _green, shape: BoxShape.circle)),
          SizedBox(width: t.xs), Text('SYNC LOCKED', style: t.textCaption.copyWith(color: _green, letterSpacing: 1.5)),
        ]),
      ),
      SizedBox(width: t.lg),
      ...
      const Spacer(),
      SizedBox(width: t.u * 10, height: t.u * 2.2, child: const _Sparkline(seed: 4, color: Color(0xFF5AB0E0))),
      SizedBox(width: t.sm),
      stat('AGC', '1.53×'),
    ]);
  }
...
class _CaptureChain extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final t = GridProvider.of(context);
    return Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
      Text('CAPTURE CHAIN', style: t.textCaption.copyWith(letterSpacing: 1.5, color: _dim)),
      SizedBox(height: t.sm),
      Expanded(child: Column(children: [
        for (final s in stages)
          Expanded(child: Padding(
            padding: EdgeInsets.only(bottom: t.xs),
            child: _ChainNode(label: s.$1, sub: s.$2, stage: s.$3, active: s.$4),
          )),
      ])),
    ]);
  }
...
class _CaptureCanvas extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
      Row(children: [
        Text('CAPTURED SIGNAL', style: t.textCaption.copyWith(letterSpacing: 1.5, color: _dim)),
        const Spacer(),
        Text('drag DE edges to set active area', style: t.textCaption.copyWith(color: _dim)),
      ]),
      SizedBox(height: t.sm),
      Expanded(child: NeumorphicInset(
        baseColor: const Color(0xFF0C0C0E),
        padding: EdgeInsets.all(t.xs),
        child: AspectRatio(
          aspectRatio: 16 / 9,
...
class _DeWindowPainter extends CustomPainter {
  ...
  @override
  void paint(Canvas canvas, Size size) {
    final de = Rect.fromLTRB(size.width * 0.05, size.height * 0.06, size.width * 0.95, size.height * 0.95);
    // dim outside the detected active area
    final dim = Path()..addRect(Offset.zero & size)..addRect(de)..fillType = PathFillType.evenOdd;
    canvas.drawPath(dim, Paint()..color = Colors.black.withValues(alpha: 0.5));
    canvas.drawRect(de, Paint()..style = PaintingStyle.stroke..strokeWidth = 1..color = _cyan);
    // edge-centred handles (DE is set per-edge, not per-corner)
    final hp = Paint()..color = _cyan;
    for (final c in [de.centerLeft, de.centerRight, de.topCenter, de.bottomCenter]) {
      canvas.drawRect(Rect.fromCenter(center: c, width: 9, height: 9), hp);
    }
    // H/V sync position ticks along top + left rulers
    final tick = Paint()..color = _cyan.withValues(alpha: 0.5)..strokeWidth = 1;
    for (int i = 0; i <= 16; i++) {
      final x = de.left + de.width * i / 16;
      canvas.drawLine(Offset(x, de.top), Offset(x, de.top + (i % 4 == 0 ? 7 : 4)), tick);
    }
    // label
    final tp = TextPainter(
      text: TextSpan(text: '1920 × 1080  active', style: TextStyle(color: _cyan, fontFamily: 'DINPro', fontSize: size.height * 0.035)),
      textDirection: TextDirection.ltr)..layout();
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.stretch, mainAxisSize: MainAxisSize.min, children: [
        ...
        Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Expanded(flex: 3, child: Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
            SizedBox(height: t.u * 6.5, child: CustomPaint(painter: _WaveformPainter(), size: Size.infinite)),
            SizedBox(height: t.xs * 0.5),
            Text('WAVEFORM · luma', style: t.textCaption.copyWith(color: _dim, fontSize: t.u * 0.8)),
          ])),
          SizedBox(width: t.sm),
          Expanded(flex: 2, child: Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
            AspectRatio(aspectRatio: 1, child: CustomPaint(painter: _VectorscopePainter())),
            SizedBox(height: t.xs * 0.5),
            Text('VECTOR', style: t.textCaption.copyWith(color: _dim, fontSize: t.u * 0.8)),
          ])),
        ]),
        SizedBox(height: t.md),
        ...
        SizedBox(height: t.xs),
        KnobPanel(null, const [('H Phase', 0.5), ('V Phase', 0.5), ('LLC Phase', 0.3), ('Coast', 0.4)], bipolar: true, knobSize: t.knobSm),
      ]),
    );
  }

  Widget _deField(GridTokens t, String label, String value, {bool accent = false}) => Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          Text(label, style: t.textCaption.copyWith(color: _dim, fontSize: t.u * 0.8, letterSpacing: 1)),
          SizedBox(height: t.xs * 0.4),
          Text(value, style: t.textValue.copyWith(
              color: accent ? const Color(0xFF5AB0E0) : _ink, fontSize: t.u * 1.7, fontWeight: FontWeight.w600)),
        ],
      );
}
...
class _WaveformPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    final r = Offset.zero & size;
    canvas.drawRRect(RRect.fromRectAndRadius(r, const Radius.circular(4)), Paint()..color = const Color(0xFF0A0E0A));
    // IRE graticule
    final g = Paint()..color = const Color(0xFF2A3A2A)..strokeWidth = 0.75;
    for (int i = 0; i <= 4; i++) {
    ...
    final trace = Paint()..color = _green.withValues(alpha: 0.5)..strokeWidth = 1;
    for (int x = 0; x < size.width; x += 2) {
      final fx = x / size.width;
      final base = 0.5 - 0.5 * math.cos(math.pi * fx * 2) * 0.6;
      for (int s = 0; s < 3; s++) {
        final y = (base + (rnd.nextDouble() - 0.5) * 0.5).clamp(0.03, 0.97) * size.height;
        canvas.drawPoints(ui.PointMode.points, [Offset(x.toDouble(), y)], trace);
      }
    }
  }
  @override
...
class _VectorscopePainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    canvas.drawCircle(c, r - 1, Paint()..style = PaintingStyle.stroke..strokeWidth = 0.75..color = const Color(0xFF2A3A2A));
    canvas.drawCircle(c, r * 0.5, Paint()..style = PaintingStyle.stroke..strokeWidth = 0.5..color = const Color(0xFF2A3A2A));
    // colour target boxes (R,Yl,G,Cy,B,Mg)
    const targets = [Color(0xFFE05A5A), Color(0xFFE0D050), Color(0xFF5AD070), Color(0xFF50C0D0), Color(0xFF6060E0), Color(0xFFD060C0)];
    for (int i = 0; i < 6; i++) {
      final a = -math.pi / 2 + i * math.pi / 3;
      final p = c + Offset(math.cos(a), math.sin(a)) * r * 0.72;
      canvas.drawRect(Rect.fromCenter(center: p, width: 4, height: 4),
          Paint()..style = PaintingStyle.stroke..strokeWidth = 0.75..color = targets[i].withValues(alpha: 0.7));
    }
    // signal blob
